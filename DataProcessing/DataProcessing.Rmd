---
title: "Автоматизация обработки данных на R"
...
*Для наглядности описательные статистики одних и тех же переменных каждой из групп расположены рядом (в столбцах)*

```{r results = F}
data <- tibble(read_excel('data_excel.xlsx'))
summary(data)
...
```
*Примечание: для групп данные представлены в формате "количество наблюдений в выборке / процент по выборке"*
...
Группы сравнивались с помощью t-теста.

Введём дополнительные гипотезы:

Н0 - группы 1 и 2 не отличаются между собой по признаку *x*

H1 - группы 1 и 2 отличаются между собой по признаку *x*
...
df1 <-
  ...
  dplyr::summarise(across(where(is.numeric), ~ t.test(.x ~`Группа`, data)$p.value %>% round(3))) %>% 
  t() %>% 
  as.data.frame() %>% 
  rownames_to_column("Переменная") %>%
  dplyr::rename('p-уровень'='V1') %>%
  mutate(`Гипотеза` = ifelse(`p-уровень` >= 0.05, 'H0', 'H1')) %>%
  flextable() %>%
  theme_box() %>%
  flextable::align(j = c(2:3), align = "center", part = 'all') %>%
  color(i = ~ `p-уровень` < 0.05, color = 'darkred') %>%
  color(i = ~ `p-уровень` >= 0.05, color = 'forestgreen') %>%
  set_table_properties(layout = "autofit")
df1
```


-   зелёным цветом выделены признаки, по которым значимые различия между группами 1 и 2 не были обнаружены
-   красным цветом выделены признаки, по которым значимые различия между группами 1 и 2 были обнаружены
